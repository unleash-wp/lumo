...
| --- | --- |
| **Currency Guard** (`wp-currency-guard`): stale `block.json` / `theme.json` / Interactivity / WP 7.0 currency (apiVersion 3, viewScriptModule, experimental supports, Block Bindings, `watch()`, router nav deprecation, render field, theme.json v3 opt-outs, html5 script / PHP 7.4 floor) | Hosted Pro MCP |
