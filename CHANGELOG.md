...
- WooCommerce Subscriptions detection wired into the pattern registry with Pro coverage flag
...
- Premium agency plugin detection: ACF Pro, Elementor Pro, Gravity Forms, Meta Box, Carbon
  ...
  each surfaces a measured upgrade teaser (or an honest detection note when Pro coverage
  is not yet curated)
...
- Snapshot locked to generated artifact: `data/snapshot.json` is never hand-edited;
  byte-parity CI guard enforces provenance
...
- HPOS order-access entry (`woocommerce-hpos-order-access`): `get_post_meta()` /
  `get_posts(['post_type' => 'shop_order'])` vs. `wc_get_order()` with dated source and
